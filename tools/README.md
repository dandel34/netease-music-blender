...
| `test_netease_offline.py` | 离线单元测试（72 项）：数据标准化、私人雷达三级降级、Cookie 处理、三种协议构造、音质回退、收藏降级、**LRC 歌词解析**、**缓存上限淘汰** | 否 | 否 |
| `test_netease_client.py` | 匿名拉真实数据（每日推荐 / 歌单详情 / 播放直链 / 歌词），打印命中的协议链路 | 否 | 是 |
| `test_in_blender.py` | 真机集成测试（25 步）：真实 `addon_enable` 启用、加密、联网、下载、`aud` 播放、面板绘制冒烟、**图标名校验**、**中文歌词字体度量**、**浮层布局与注册**、**缓存上限**、**音频设备参数与内存缓存**、收藏、自动下一首 | **是** | 部分是 |
